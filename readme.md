...
- `rating_allocation_mode`: How narratives will be allocated to raters. Options are:
    ...
    - `'min-per-combo'`: everyone will rate some minimum number (set by `min_per_combo`, below) of narratives for each combination of perspective expressed in the narrative and perspective held by the writer (no rater rates their own narrative); note that to use this option, `query_participant_perspective` must be set to `true`
